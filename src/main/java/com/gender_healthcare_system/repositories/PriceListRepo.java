...
public interface PriceListRepo extends JpaRepository<PriceList, Integer> {
    ...
    @Modifying
    @Query("UPDATE PriceList p SET p.price = :#{#payload.price}, " +
            "p.description = :#{#payload.description} " +
            "WHERE p.priceId = :id")
    void updatePriceListById(@Param("id") int id, @Param("payload") PriceListUpdatePayload payload);

    @Modifying
    @Query("DELETE FROM PriceList p WHERE p.priceId = :id")
